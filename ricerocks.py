...
class Ship:
    ...
    def turn(self,direction):
        ...
        self.angle_vel = 0.1 * self.direction   
            
    def thrusting(self, thrust):
        if thrust:
            self.thrust = True
    ...
    def shoot(self):
        ...
        missile_vel = [self.vel[0] + self.forward[0] * 6, self.vel[1] + self.forward[1] * 6]
...
def draw(canvas):
    ...
    canvas.draw_text('Score', [690, 50], 30, 'White')
    canvas.draw_text(str(score), [690, 80], 30, 'White')
    ...
    # check if missiles have hit any rocks
    if group_group_collide(missile_group, rock_group):
        score += 1
    
    # check if ship has hit any rocks
...
# helper fuction to call/update sprite groups        
...
def rock_spawner():
    global rock_group, score, WIDTH, HEIGHT, my_ship
    pos = [0, 0]
    vel = [0, 0]
    ang_vel = 0
    vel_lower = -1
    ...
    if not started:
        return
    if dist(pos, my_ship.get_position()) <= 130:        
        return
...
a_rock = Sprite([WIDTH / 3, HEIGHT / 3], [1, 1], 0, .09, asteroid_image, asteroid_info)
